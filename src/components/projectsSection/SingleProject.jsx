...
const SingleProject = ({name,year,align,image,link}) => {
  return (
    <div className={ `flex w-full sm:flex-col-reverse items-center gap-8 ${
        ...
        <div>
           <h2 className="md:text-3xl sm:text-2xl text-lightAvacadoGreen">{name}</h2>
           <h2 className={`text-xl font-thin text-sageTintedOlive font-special sm:text-center ${align === "left" ? "md:text-right" : "md:text-left"
           }`}>
             {year}
           </h2>
           <a href={link} className={`text-lg flex gap-2 items-center text-oliveGreen hover:text-richMetallicGold transition-all duration-500 cursor-pointer sm:justify-self-center ${
